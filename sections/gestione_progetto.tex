...
\strong{dipendenza temporale}.

PERT utilizza la seguente terminologia:

\begin{itemize}
  \item \strong{PERT event}: punto che demarca l'inizio o la fine di una o più
    attività;
  \item \strong{PERT activity}: svolgimento di un'attività, che consuma risorse.
    Il suo completamento risulta nel passaggio da un evento all'altro;
  \item \strong{\frgnword{Float}} o \strong{\frgnword{slack}}: relativo a un
    singolo \frgnword{task}, rappresenta quanto ritardo in termini di tempo può
    avere quel \frgnword{task} senza causare ritardi ai \frgnword{task} seguenti
    (\frgnword{free float}) o all'intero progetto (\frgnword{total float});
  \item \strong{Cammino critico}: il cammino più lungo possibile dall'evento
