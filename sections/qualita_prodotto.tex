...
\strong{strumento di quantificazione} del prodotto e del processo, e sono
...
\begin{itemize}
  \item \strong{Parametri per metodo}: metodi con un elevato numero di parametri
    rischiano di portare a un elevato accoppiamento, e sono più complessi da
    comprendere e di conseguenza da realizzare;
  \item \strong{Number of children (NOC)}: quantifica il numero di figli diretti
  ...
  \item \strong{Depth of Inheritance Tree (DIT)}: indica la massima profondità
  ...
  \item \strong{\frgnword{Afferent Coupling}}: dato un package, indica il numero
  ...
  \item \strong{\frgnword{Efferent Coupling}}: dato un package, indica il numero
...
Sia $a$ il valore di \frgnword{Afferent Coupling} di un package, e $e$ il suo
valore di \frgnword{Efferent Coupling}. La seguente metrica indica il grado di
...
\begin{itemize}
  \item \strong{Livello di annidamento}: indica il livello di profondità di
    annidamento raggiunto dalle strutture di controllo;
  \item \strong{Complessità ciclomatica}: ogni programma può essere
...
\paragraph{Metriche dei test}
\label{par:metriche_dei_test}

\begin{itemize}
  \item \strong{Copertura del codice}: indica la percentuale di codice coperta
